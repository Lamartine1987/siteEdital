...
document.addEventListener("DOMContentLoaded", function () {
  ...
  firebase.database().ref("editais").once("value").then((snapshot) => {
    const editais = snapshot.val();
    const editalEncontrado = Object.values(editais).find(e => e.id === idEdital);
    ...
    firebase.auth().onAuthStateChanged(async function (user) {
      ...
      const snapUser = await firebase.database().ref(`usuarios/${userId}`).once("value");
      ...
      if (!cargoSelecionado && !materiaSelecionada) {
        ...
        for (const nomeCargo in cargos) {
          ...
          for (const nomeMateria in materias) {
            ...
            const progressoMateria = progressoUsuario?.[nomeCargo]?.[nomeMateria] || {};
            totalEstudados += Object.values(progressoMateria).filter(v => v === true).length;
          }

          const percentual = totalAssuntos > 0 ? Math.round((totalEstudados / totalAssuntos) * 100) : 0;
          ...
          card.onclick = () => {
            window.location.href = `paginaEditais.html?id=${idEdital}&cargo=${encodeURIComponent(nomeCargo)}`;
          };
          ...
          if (["anual", "edital", "cargo"].includes(tipoUsuario)) {
            ...
            estaMatriculado = matriculados?.id === idEdital && matriculados?.cargo === nomeCargo;
          }
          ...
          const matricular = () => {
            ...
            const ref = ["anual", "edital", "cargo"].includes(tipoUsuario)
              ? firebase.database().ref(`usuarios/${userId}/matriculados/${idEdital}/${nomeCargo}`)
          ...
          const desmatricular = () => {
            const updates = ["anual", "edital", "cargo"].includes(tipoUsuario)
              ? {
                  [`usuarios/${userId}/matriculados/${idEdital}/${nomeCargo}`]: null,
                  [`usuarios/${userId}/progresso/${idEdital}/${nomeCargo}`]: null,
                }
              : {
          ...
          if (estaMatriculado) {
            ...
            btn.onclick = (e) => {
              e.stopPropagation();
              criarModalConfirmacao("Deseja mesmo desmatricular e apagar o progresso?", desmatricular);
            };
          } else {
            ...
            btn.onclick = (e) => {
              ...
              if (tipoUsuario === "gratuito") {
                ...
                if (jaMatriculado && (editalDiferente || cargoDiferente)) {
                  return bloquearMatricula("Você já está matriculado em outro cargo.");
                }
              } else if (tipoUsuario === "cargo") {
                const editaisMatriculados = Object.keys(matriculados || {});
                const jaTemOutroEdital = editaisMatriculados.some(eid => eid !== idEdital);

                if (jaTemOutroEdital) {
                  return bloquearMatricula("Com o plano atual, você só pode se matricular em um cargo de um único edital.");
                }

                const jaMatriculadoNoMesmoEdital = Object.keys(matriculados[idEdital] || {}).length > 0;
                if (jaMatriculadoNoMesmoEdital) {
                  return bloquearMatricula("Você já está matriculado em um cargo neste edital, para aproveitar as outras funcionalidades, faça o updgrade do seu plano e matricule-se em qualquer cargo do edital atual.");
                }
              } else if (
                tipoUsuario === "edital" &&
                ...
                return bloquearMatricula("Você só pode se matricular em cargos do mesmo edital.");
              }
      ...
      if (cargoSelecionado && !materiaSelecionada) {
        ...
        for (const nomeMateria in cargoData.materias) {
          ...
          const estudados = Object.values(progresso[nomeMateria] || {}).filter(v => v === true).length;
          ...
          `;
          card.onclick = () => {
            window.location.href = `assuntos.html?id=${idEdital}&cargo=${encodeURIComponent(cargoSelecionado)}&materia=${encodeURIComponent(nomeMateria)}`;
          };
